...
namespace WarehouseCoursework.PageFolder.AdminFolder
{
    ...
    /// </summary>
    public partial class GoodsInStockAddPage : Page
    {
        public GoodsInStockAddPage()
        {
            InitializeComponent();
        ...
        {
            if (string.IsNullOrWhiteSpace(AddCount.Text))
            {
            ...
            else
            {
                try
                {
                    if (int.TryParse(AddCount.Text, out int count) && (count > 0)) 
                    {
